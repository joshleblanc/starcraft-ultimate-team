class CardSetsController < ApplicationController
  ...
  def update
    respond_to do |format|
      if @card_set.update(card_set_params)
        format.html { redirect_to @card_set, notice: "Card set was successfully updated.", status: :see_other }
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to card_sets_path, notice: "Card set was successfully destroyed.", status: :see_other }
  ...
  private
    ...
    def set_card_set
      @card_set = CardSet.find(params.expect(:id))
    end

    # Only allow a list of trusted parameters through.
    def card_set_params
      params.expect(card_set: [ :name, :description ])
    end
end
